...
feature_names_post = [
    "Calcification", "Capsular Invasion", "Margin", "Diameter", "SII"
]

# ---------------------- 2. 页面基础设置 & 绝经状态选择 ----------------------
st.set_page_config(page_title="Thyroid Skip Metastasis Predictor")
st.title("Skip Metastasis Prediction Model for Papillary Thyroid Carcinoma")

# 分组选择
menopause_status = st.radio(
    "Select Patient Menopausal Status",
    options=[0, 1],
    format_func=lambda x: "0 - Premenopausal (Non-menopausal, with Age, no SII)" if x == 0 else "1 - Postmenopausal (Menopausal, with SII, no Age)"
)
...
if menopause_status == 0:
    ...
    st.subheader("Current Group: Premenopausal Women (Input Age, No SII)")
else:
    cur_model = model_post
    cur_feature_names = feature_names_post
    st.subheader("Current Group: Postmenopausal Women (Input SII, No Age)")
...
Margin = st.selectbox("Nodule Margin", options=[0, 1], format_func=lambda x: 'Clear (0)' if x == 0 else 'Ill-defined (1)')
Diameter = st.number_input("Maximum Nodule Diameter (mm)", min_value=0.1, max_value=200.0, value=10.0, step=0.1)
...
if st.button("Start Prediction & Generate SHAP Force Plot"):
    ...
    if pred_class == 1:
        advice = (
            f"Based on the {group_text} subgroup XGBoost model, this patient has a high risk of skip metastasis in papillary thyroid carcinoma. "
            f"The predicted probability of skip metastasis is {risk_percent:.1f}%. "
        ...
        advice = (
            f"Based on the {group_text} subgroup XGBoost model, this patient has a low risk of skip metastasis in papillary thyroid carcinoma. "
            f"The predicted probability of no skip metastasis is {risk_percent:.1f}%. "
